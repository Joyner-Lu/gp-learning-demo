    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf buf, List out) {
        ...
        while (buf.readableBytes() >= packetHeaderSize) {
            ...
            if (buf.readableBytes() - packetHeaderSize >= packetHeader.getDataLen()) {
                ...
                PacketContent packetContent = ObjectUtils.bytesToObj(packetBodyBytes, PacketContent.class);
                System.out.println("decode:" + packetContent);
                PacketMsg packetMsg = new PacketMsg(packetHeader, packetContent);
                out.add(packetMsg);
            } else {
                //要跳出循环
                break;
