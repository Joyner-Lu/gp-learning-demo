...
public class LuNioEngineV2 {
    ...
    public void start(int workerCount) throws Exception {
        ...
        //初始化boss
        NioThreadV2 boss = new NioThreadV2(selectors[0], Integer.MAX_VALUE, true);
        boss.start();
        //初始化worker
        for (int i = 0; i < workerCount; i++) {
            //0被boss占用
            NioThreadV2 worker = new NioThreadV2(selectors[i + 1], i, false);
            worker.start();
        }
        //阻塞
        System.in.read();
    ...
    public static void main(String[] args) throws Exception {
        LuNioEngineV2 engine = new LuNioEngineV2();
        engine.start(3);
    }
}
